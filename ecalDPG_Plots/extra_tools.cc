...
std::vector<std::pair<std::string, std::string>> getBranchList(std::string _treePath, std::string _treeName, Bool_t _sort=1, Bool_t _verbose=1);
std::vector<std::pair<std::string, std::string>> getBranchList(TTree *_tree, Bool_t _sort=1, Bool_t _verbose=1);
...
struct sample {
	...
	void assignAtt(TGraph *_hist, Float_t _markerSize=1.5, Float_t _lineWidth=2.){
		_hist->SetMarkerStyle(marker);
		_hist->SetMarkerSize(_markerSize);
		_hist->SetMarkerColor(TColor::GetColor(color.c_str()));
		_hist->SetLineColor(TColor::GetColor(color.c_str()));
...
struct parseOptions {
	...
	void parseIt(std::string _optFile){
		std::cout<<"\t\tOptions parsed from file "<<_optFile<<"... "<<std::endl;
...
TH1* getHistFromFile(std::string _histName, std::string _filename, Bool_t _verbose){
	cout<<"Reading :"<<_filename.c_str()<<endl;
	...
	// 	return nullptr;
	// }
	std::cout<<"\t\tLoading histogram "<<_histName<<" from file "<<_filename<<std::endl;
	...
	if(_verbose && _hist)std::cout<<"\t\t\tSuccess!"<<std::endl;
...
TObject *getObjectFromFile(std::string _objectName, std::string _filename){
	...
	TDirectory *_cd = gDirectory->GetDirectory("");
	// _cd->Append(_object, 0);
...
std::vector<std::pair<std::string, std::string>> getBranchList(std::string _treePath, std::string _treeName, Bool_t _sort, Bool_t _verbose){
	...
	std::vector<std::pair<std::string, std::string>> bList = getBranchList(bChain, _sort, _verbose);
...
std::vector<std::pair<std::string, std::string>> getBranchList(TTree *bChain, Bool_t _sort, Bool_t _verbose){
	TObjArray *bArray = (TObjArray*)bChain->GetListOfBranches()->Clone();
	bArray->SetOwner(kFALSE);

	if(_sort) bArray->Sort();

	std::string _treePath = bChain->GetCurrentFile()->GetName();

	std::vector<std::pair<std::string, std::string>> bList;

	std::cout<<"List of branches in TTree "<<bChain->GetName()<<" in file "<<_treePath<<":"<<std::endl;
	TIter iBr(bArray);
